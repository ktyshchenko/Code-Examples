...
public class TrollController : MonoBehaviour
{
    ...
    private const int animMovingInt = 1;
    private const int animCheeringInt = 6;
    //private const int animJumpingInt = 7;
    private const int animHittingInt = 5;
    ...
    private void DamageTown()
    {
        if (GameManager.isGameOver == false)
        {
            ...
            if (transform.localPosition.z <= -zBottomBound + offset)
            {
            ...
            if (transform.localPosition.z <= -zBottomBound)
            {
